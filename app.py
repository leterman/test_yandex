...
def add_new_shelf(shelf_number, shelf_num):
    if not shelf_number:
        shelf_number = input(shelf_num)
...
def append_doc_to_shelf(doc_number, shelf_number):
    add_new_shelf(shelf_number, 1)
    directories[shelf_number].append(doc_number)


def delete_doc(user_doc_number):
    # user_doc_number = input(user_doc_num)
    doc_exist = check_document_existance(user_doc_number)
    if doc_exist:
        for current_document in documents:
            doc_number = current_document['number']
...
user_doc_num = '2207 876234'
def get_doc_shelf(user_doc_num):
    user_doc_number = input(user_doc_num)
    doc_exist = check_document_existance(user_doc_number)
    if doc_exist:
        for directory_number, directory_docs_list in directories.items():
...
def move_doc_to_shelf(user_doc_num, shelf_num ):
    user_doc_number = input(user_doc_num)
    user_shelf_number = input(shelf_num)
...
def show_document_info(document):
    ...
    xx = ('{} "{}" "{}"'.format(doc_type, doc_number, doc_owner_name))
    return xx
...
new_num = '23231'
new_type = 'ID'
new_doc_owner = 'Ivan Ivanov'
new_doc_shelf = '1'
def add_new_doc(new_num, new_doc_owner, new_type, new_doc_shelf):
    new_doc_number = new_num
    new_doc_type = new_type
    new_doc_owner_name = new_doc_owner
    new_doc_shelf_number = new_doc_shelf
    new_doc = {
        "type": new_doc_type,
...
# add_new_doc(new_num,new_doc_owner,new_type,new_doc_shelf)
